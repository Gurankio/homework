...
public abstract class DynamicMenu {
    ...
    // Static Menu Shothands

    public static <T> T choose(String message, List<String> optionsList, Class<?> type, List<T> outs) {
        ...
        new DynamicMenu() {
            ...
            protected void setup() {
                message(message);
                option(String.join("\n", options), " @ ", type, t -> {
                    out.set(optionsList.indexOf(optionsList.stream().filter(option -> option.contains("[" + t.toString() + "]")).findFirst().orElseThrow()));
                    return false;
                });
            }
        }.run();
        return outs.get(out.get());
    }
    ...
    protected void listln(String header, Stream<String> elements) {
       println(header);
       elements.forEach(string -> println(Logger.wrap(string, " + ")));
    }
